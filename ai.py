...
async def get_recommendations(api_key: str, description: str, tickers: List[str], data: dict):
    ...
    return await client.models.generate_content(
        ...
        contents=(
            ...
            f'{", ".join(tickers)}. Here is some data we have compiled about them: {str(data)}. '
            'Narrow down the number of stocks, provide a rationale based on the data and nature '
            'of the company for each stock you recommend, and provide the dict keys for each metric '
            'you used to reach this conclusion.'
        ),
        config={
            'response_mime_type': 'application/json',
